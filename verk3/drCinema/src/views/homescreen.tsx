...
export default function HomeScreen() {
	...
	const filtered = useMemo(() => {
		...
		if (filters?.actors?.trim()) {
			...
			result = result.filter((m) =>
				actorNames.some((actor) => m.actors_abridged.some((a) => a.name.toLowerCase().includes(actor)))
			);
		}
		...
		if (filters?.directors?.trim()) {
			...
			result = result.filter((m) =>
				directorNames.some((director) => m.directors_abridged.some((d) => d.name.toLowerCase().includes(director)))
			);
		}
