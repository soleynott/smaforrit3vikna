...
export function List() {
  ...
  const [confettiForTaskId, setConfettiForTaskId] = useState<number | null>(
    null,
  );
  ...
  const handleTaskToggle = (taskId: number) => {
    ...
    if (willBeFinished) {
      ...
      setTimeout(
        () => setConfettiForTaskId((prev) => (prev === taskId ? null : prev)),
        5000,
      );
    }
  };
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      {confettiForTaskId !== null && (
        <Confetti
          key={`confetti-${confettiForTaskId}`}
          count={80}
          origin={{ x: -10, y: 0 }}
        />
      )}
